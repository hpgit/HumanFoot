...
class Callback:
    ...
    def simulateCallback(self, frame):
        ...
        for i in range(int(stepsPerFrame)):
            ...
            ddq_pure = None
            invM = dartModel.skeleton.inv_mass_matrix()
            cori = dartModel.skeleton.coriolis_and_gravity_forces()
            if torque_None:
                dartModel.skeleton.set_forces(pdController.compute())
                ddq_pure = np.dot(invM, pdController.compute() - cori)
            # elif i%5 == 0:
            else:
                dartModel.skeleton.set_forces(torques)
                ddq_pure = np.dot(invM, torques - cori)

            print('dq_des: ', dartModel.skeleton.dq + wcfg.timeStep * ddq_pure)
            dartModel.step()
            print('dq: ', dartModel.get_dq())
