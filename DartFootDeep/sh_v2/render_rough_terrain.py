...
def main():
    ...
    if not MOTION_ONLY:
        ...
        for shape_idx in range(ppo.env.world.skeletons[0].body(0).num_shapenodes()//2):
            control_model_renderer.geom_colors[0][0][shape_idx] = (128, 128, 128)

        # viewer.doc.addRenderer('contact', yr.VectorsRenderer(rd_contact_forces, rd_contact_positions, (255,0,0)))
        viewer.doc.addRenderer('extraForceEnable', yr.WideArrowRenderer(rd_exf, rd_exf_pos, (255,0,0), lineWidth=.05, fromPoint=False))
    ...
    def simulateCallback(frame):
        ...
        if PUSH and 110 < frame < 123:
            ...
            rd_exf_pos.append(ppo.env.skel.body('Spine').to_world(ppo.env.applied_force_offset))
