...
from DartDeep.sh_multi_v2.dart_multi_env_v2 import HpDartMultiEnv
...
class PPO_MULTI(object):
    def __init__(self, env_name, num_slaves=1, eval_print=True, eval_log=True, visualize_only=False):
        ...
        self.motion_num = self.env.motion_num
    ...
    def envs_send_motion_nums(self):
        for motion_sender in self.motion_sender:
            motion_sender.send(random.randrange(0, self.motion_num))

    def envs_send_motion_num(self, proc_idx):
        self.motion_sender[proc_idx].send(random.randrange(0, self.motion_num))
