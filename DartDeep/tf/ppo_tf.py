...
class BasePPO(object):
    def __init__(self, action_space, observation_space,scope, cliprange=0.2, environment='default'):
        self.scope = scope
        ...
        self.checkpoint_path = './save_model/'+environment
...
class MlpPPO(BasePPO):
    def __init__(self, action_space, observation_space, scope, cliprange=0.2, environment='default'):
        super().__init__(action_space, observation_space, scope, cliprange=cliprange, environment=environment)
        self.build_net()

    def build_critic_net(self, scope):
        with tf.variable_scope(scope):
            dl1 = tf.contrib.layers.fully_connected(inputs=self.s, num_outputs=1024,
                                                    activation_fn=tf.nn.relu,
                                                    scope='dl1')

            dl2 = tf.contrib.layers.fully_connected(inputs=dl1, num_outputs=512,
                                                    activation_fn=tf.nn.relu,
                                                    scope='dl2')
    ...
    def build_actor_net(self, scope, trainable):
        with tf.variable_scope(scope):
            ...
            mu = 2 * tf.contrib.layers.fully_connected(inputs=dl2, num_outputs=self.num_action,
                                                    activation_fn=tf.nn.tanh,
                                                    trainable = trainable,
                                                   scope='mu')
            sigma = tf.contrib.layers.fully_connected(inputs=dl2, num_outputs=self.num_action,
