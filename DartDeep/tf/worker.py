...
class Worker(object):
    def __init__(self, name, env, summary_writer, global_episodes,
                 visualize=True,
                 gamma=0.99, batch_size=32, training_step=10, a_lr=1e-4, c_lr=1e-4):
        self.scope = name
        self.env = env
        ...
        self.ppo = MlpPPO(self.action_space, self.observation_space, self.scope, environment=name)

        self.gamma = gamma
        self.batch_size = batch_size
        self.training_step = training_step
        self.train_op_actor = tf.train.AdamOptimizer(a_lr).minimize(self.ppo.actor_loss)
        self.train_op_critic = tf.train.AdamOptimizer(c_lr).minimize(self.ppo.critic_loss)
    ...
    def process(self, sess, saver):
        ...
        while True:
            ...
            for i in range(0, self.batch_size):
                ...
                x_t = x_t1
                if self.terminal:
                    print('ID :' + self.scope + ', global episode :' + str(global_episodes)+ ', episode length :' + str(episode_length)+ ', total reward :' + str(total_rewards))
                    x_t = self.env.reset()
                    summary = tf.Summary()
