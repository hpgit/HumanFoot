...
import mtInitialize_008 as mit
...
#    viewer.doc.addRenderer('rd_dH_des', yr.VectorsRenderer(rd_dH_des, rd_CM, (0,255,0)))
    ...
    def simulateCallback(frame):
        ...
        p3 = controlModel.getBodyPositionGlobal(indexFootL[4])
        p4 = controlModel.getBodyPositionGlobal(indexFootR[4])
        ...
        if footPartNum == 1:
            ...
            r = 1.
            desFCL = (controlModel.getBodyPositionGlobal(supL)*r + controlModel.getBodyPositionGlobal(indexFootL[2])*(1.0-r))#controlModel.getBodyPositionGlobal(indexFootL[1])
            desFCR = (controlModel.getBodyPositionGlobal(supR)*r + controlModel.getBodyPositionGlobal(indexFootR[2])*(1.0-r))#controlModel.getBodyPositionGlobal(indexFootR[1])
        ...
        vecRatio = mm.length(curRelCMVec)*10.
            ...
            for i in range(1, footPartNum) :
                if i == i :
                    if contactFlagFootL[i] == 1:
                        desLinearAccL, desForePosL = getDesFootLinearAcc(motionModel, controlModel, indexFootL[i], ModelOffset, CM_ref, CM, Kk2, Dk2, 0.04) 
                    ...
                    if contactFlagFootR[i] == 1:
                        desLinearAccR, desForePosR = getDesFootLinearAcc(motionModel, controlModel, indexFootR[i], ModelOffset, CM_ref, CM, Kk2, Dk2, 0.04) 
