...
import mtInitialize_002 as mit
...
#    motion, mcfg, wcfg, stepsPerFrame, config = mit.create_vchain_5()
    ...
    rearSupRJointMasks = [yjc.getLinkJointMask(motion[0].skeleton, rearSupR)]
...
#    viewer.doc.addRenderer('rd_dH_des', yr.VectorsRenderer(rd_dH_des, rd_CM, (0,255,0)))
    ...
    def simulateCallback(frame):
        ...
        if (flagSupLContact | flagForeSupLContact | flagRearSupLContact | flagSupRContact | flagForeSupRContact | flagRearSupRContact) == 0:
            footCenter = footCenter
        elif (flagSupLContact | flagForeSupLContact | flagRearSupLContact) == 0:
            footCenter = footCenterR
        elif (flagSupRContact | flagForeSupRContact | flagRearSupRContact) == 0:
        ...
        if (flagForeSupLContact | flagForeSupRContact | flagRearSupLContact | flagRearSupRContact) == 1 :
            Kk2 = Kk * 1.0
            
        Dk2 = 2*(Kk2**.5)
        if ((flagForeSupLContact | flagForeSupRContact | flagRearSupLContact | flagRearSupRContact) >= 0):
        ...
        if flagForeSupLContact == 1 :
            desLinearAccL, desForePosL = getDesFootLinearAcc(motionModel, controlModel, foreSupL, ModelOffset, CM_ref, CM, Kk2, Dk2) 
            desAngularAccL = getDesFootAngularAcc(motionModel, controlModel, foreSupL, Kk2, Dk2)
            if len(Jsup_2) > 1 :
                a_sup_2 = np.hstack(( a_sup_2, np.hstack((desLinearAccL, desAngularAccL)) ))
                Jsup_2 = np.vstack(( Jsup_2, JforeSupL ))
                dJsup_2 = np.vstack(( dJsup_2, dJforeSupL ))
            else :
                a_sup_2 = np.hstack((desLinearAccL, desAngularAccL))
                Jsup_2 = JforeSupL.copy()
                dJsup_2 = dJforeSupL.copy()
        if flagForeSupRContact == 1:
            desLinearAccR, desForePosR = getDesFootLinearAcc(motionModel, controlModel, foreSupR, ModelOffset, CM_ref, CM, Kk2, Dk2) 
            desAngularAccR = getDesFootAngularAcc(motionModel, controlModel, foreSupR, Kk2, Dk2)
            if len(Jsup_2) > 1 :
                a_sup_2 = np.hstack(( a_sup_2, np.hstack((desLinearAccR, desAngularAccR)) ))            
                Jsup_2 = np.vstack(( Jsup_2, JforeSupR ))
                dJsup_2 = np.vstack(( dJsup_2, dJforeSupR ))
            else :
                a_sup_2 = np.hstack((desLinearAccR, desAngularAccR))
                Jsup_2 = JforeSupR.copy()
                dJsup_2 = dJforeSupR.copy()
            
        if flagRearSupLContact == 1 :
            desLinearAccL, desRearPosL = getDesFootLinearAcc(motionModel, controlModel, rearSupL, ModelOffset, CM_ref, CM, Kk2, Dk2) 
            desAngularAccL = getDesFootAngularAcc(motionModel, controlModel, rearSupL, Kk2, Dk2)
            if len(Jsup_2) > 1 :
                a_sup_2 = np.hstack(( a_sup_2, np.hstack((desLinearAccL, desAngularAccL)) ))            
                Jsup_2 = np.vstack(( Jsup_2, JrearSupL ))
                dJsup_2 = np.vstack(( dJsup_2, dJrearSupL ))
            else :
                a_sup_2 = np.hstack((desLinearAccL, desAngularAccL))
                Jsup_2 = JrearSupL.copy()
                dJsup_2 = dJrearSupL.copy()
        if flagRearSupRContact == 1:
            desLinearAccR, desRearPosR = getDesFootLinearAcc(motionModel, controlModel, rearSupR, ModelOffset, CM_ref, CM, Kk2, Dk2) 
            desAngularAccR = getDesFootAngularAcc(motionModel, controlModel, rearSupR, Kk2, Dk2)
            if len(Jsup_2) > 1 :
                a_sup_2 = np.hstack(( a_sup_2, np.hstack((desLinearAccR, desAngularAccR)) ))
                Jsup_2 = np.vstack(( Jsup_2, JrearSupR ))
                dJsup_2 = np.vstack(( dJsup_2, dJrearSupR ))
            else:
                a_sup_2 = np.hstack((desLinearAccR, desAngularAccR))
                Jsup_2 = JrearSupR.copy()
                dJsup_2 = dJrearSupR.copy()
