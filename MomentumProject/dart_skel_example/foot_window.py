...
class FootWindow(Fl_Window):
    def __init__(self, x, y, w, h, title, model):
        ...
        self.check_op_l = Fl_Check_Button(10, 10, 30, 30, 'T')
        self.check_h_l = Fl_Check_Button(30, 90, 30, 30, 'H')

        self.check_op_r = Fl_Check_Button(150, 10, 30, 30, 'T')
    ...
    def get_contact_state(self):
        contact = 0
        contact = contact+1 if self.check_h_r.value() or self.check_op_r.value() else contact+0
        contact = contact+2 if self.check_h_l.value() or self.check_op_l.value() else contact+0
