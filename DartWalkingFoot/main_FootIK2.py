...
SEGMENT_FOOT = True
SEGMENT_FOOT_MAG = .03
SEGMENT_FOOT_RAD = SEGMENT_FOOT_MAG/2.
...
def walkings(params, isCma=True):
    ...
    if not isCma:
        ...
        def offsetSliderCallback(slider):
            ...
            hfi.footAdjust(motion_ori[0], footIdDic, SEGMENT_FOOT_MAG, SEGMENT_FOOT_RAD, -0.01)
            bodyNum = dartMotionModel.getBodyNum()


            dartMotionModel.update(motion_ori[0])
            bodyIDs, positions, positionLocals, forces = dartMotionModel.calcPenaltyForce(list(range(bodyNum)), 1.*np.ones(bodyNum), 1000., 10.)
            del rd_cPositions[:]
            del rd_cForces[:]
            forces100 = [forces[i]/100. for i in range(len(forces))]

            rd_cPositions.extend(positions)
            rd_cForces.extend(forces100)
