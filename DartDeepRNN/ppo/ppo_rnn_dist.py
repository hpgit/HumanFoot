...
def worker(env_name, proc_num, result_sender, action_receiver):
    """
...
class PPO(object):
    ...
    def init_envs(self):
        for slave_idx in range(self.num_slaves):
            r_s, r_r = Pipe()
            a_s, a_r = Pipe()
            p = Process(target=worker, args=(self.env_name, slave_idx, r_s, a_r))
    ...
    def env_get_states(self):
        for slave_idx in range(self.num_slaves):
            self.result_receiver[slave_idx].recv()
            pass
