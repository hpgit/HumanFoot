...
import mtInitialize_teaser as mit
...
def getBodyGlobalPos(model, motion, jointIndex):
    return model.getBodyPositionGlobal(jointIndex)

def getBodyGlobalOri(model, motion, jointIndex):
    return model.getBodyOrientationGlobal(jointIndex)
...
#    motion, mcfg, wcfg, stepsPerFrame, config = mit.create_vchain_5()
    ...
    def simulateCallback(frame):
        ...
        print 'Joint Position(Global)'
        for i in range(0, (totalDOF-3)/3):
            pos = motion[0].getJointPositionGlobal(i)

            print motion[0].skeleton.getJointName(i), pos[0], pos[1], pos[2]

        print 'Joint Orientation(Global)'
        for i in range(0, (totalDOF-3)/3):
            ori = motion[0].getJointOrientationGlobal(i)
            print motion[0].skeleton.getJointName(i), ori

        print 'Box Position(Global)'
        for i in range(0, (totalDOF-3)/3):
            pos = getBodyGlobalPos(controlModel, motion, i)
            print motion[0].skeleton.getJointName(i), pos[0], pos[1], pos[2]
            

        print 'Box Orientation(Global)'
        for i in range(0, (totalDOF-3)/3):
            ori = getBodyGlobalOri(controlModel, motion, i)
            print motion[0].skeleton.getJointName(i), ori
