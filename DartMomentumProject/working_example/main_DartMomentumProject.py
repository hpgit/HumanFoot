...
def main():
    ...
    def simulateCallback(frame):
        ...
        ddth_des_flat = pdcontroller.compute()
        ...
        #calculate jacobian
        Jsys = np.zeros((6*dartModel.getBodyNum(), dartModel.getTotalDOF()))
        dJsys = np.zeros((6*dartModel.getBodyNum(), dartModel.getTotalDOF()))
        for i in range(dartModel.getBodyNum()):
            Jsys[6*i:6*i+6, :] = dartModel.getBody(i).world_jacobian()
            dJsys[6*i:6*i+6, :] = dartModel.getBody(i).world_jacobian_classic_deriv()

        JsupL = dartModel.getBody(supL).world_jacobian(footCenterL)
        dJsupL = dartModel.getBody(supL).world_jacobian_classic_deriv(footCenterL)

        JsupR = dartModel.getBody(supR).world_jacobian(footCenterR)
        dJsupR = dartModel.getBody(supR).world_jacobian_classic_deriv(footCenterR)
